...
var VersionTitle = "Sound Board Plus Preview 1";
var VersionNumber = "Preview 1";
var ContinuityVersionNumber = "1.0"
var BuildNumber = 1;
var CopyrightTitle = "Content By ElmerF 2022";
var ELMSUIVersion = "1.4.2";
var CompilationDate = "September 7, 2022";
...
function OnloadTasks(){	
	...
	switch (PageName){
		...
		SBP_CheckFileExistence();
		enableGreetings = 0;
		break;
		case "SBP_BoardEditor.html":
...
function check_Version(){
	var versionFile = localStorage.getItem("SBP_Version");
	if (versionFile){ //Version file exists
		versionNumber = localStorage.getItem("SBP_Version");
		var versionFile_Count = Object.keys(JSON.parse(localStorage.getItem("SBP_Version"))).length;
		var versionNumber = [];
		var versionFile_Data = Object.values(JSON.parse(localStorage.getItem("SBP_Version")));
		...
		if (versionNumber[1] != null){
			if (versionNumber[1] != BuildNumber){
				...
				document.getElementById("subwindow_UpdateWindow").style.display = "block";
				const Version = [ContinuityVersionNumber, BuildNumber];
				localStorage.setItem("SBP_Version", JSON.stringify(Version));
			}
		} else {
			const Version = [ContinuityVersionNumber, BuildNumber];
			localStorage.setItem("SBP_Version", JSON.stringify(Version));
...
function toggle_SearchBar(){
	...
	if (SearchBar.style.display == "none"){
		...
		console.log(document.getElementById("pageElement_SearchQuery_3").focus);
		console.log(document.getElementById("pageElement_SearchQuery_3") === document.activeElement);
		} else {
	...
	if (Behavior_DisplayToasts == true){
		...
		switch(type){
			...
			case "Settings_FormEmpty": //When a shortcut in Shortcut Editor is successfully created
				...
				break;
			case "SBP_Pause": //When a shortcut in Shortcut Editor is successfully created
				toast_Icon.setAttribute("src", "Assets/Icons/icon_Placeholder.png");
				toast_Title.innerHTML = "Paused all sounds";
				toast_Subtitle.innerHTML = "All currently playing sounds have been paused.";
				break;
			case "SBP_Play": //When a shortcut in Shortcut Editor is successfully created
				toast_Icon.setAttribute("src", "Assets/Icons/icon_Placeholder.png");
				toast_Title.innerHTML = "Played all sounds";
				toast_Subtitle.innerHTML = "All currently paused sounds have been resumed playing.";
				break;
			case "SBP_Stop": //When a shortcut in Shortcut Editor is successfully created
				toast_Icon.setAttribute("src", "Assets/Icons/icon_Placeholder.png");
				toast_Title.innerHTML = "Stopped all sounds";
				toast_Subtitle.innerHTML = "All currently playing sounds have been stopped.";
				break;
...
function SE_ItemSelection(ID){
	switch(ID){
		...
		document.getElementById('SE_AddItem_Description').innerHTML = "Clicking a sound button plays the sound that it's linked to. When entering the file name, make sure that you include the file extension (.mp3, .ogg, etc.) and must be placed inside Assets/Sounds.";
...
function check_SettingsFileExistence(){
	if(localStorage.getItem("DL_Settings_Appearance") == null || localStorage.getItem("DL_Settings_Behaviors") == null){
		const Settings_Appearance_Obj = {
			appearance_Wallpaper_Img: "url(../Assets/Background/default_darkmode.jpg)",
			AP_CL_AccentColor: "#cf5520",
			AP_CL_AccentColor_Hover: "#a03d13",
			AP_CL_BGColor_General: "#171010",
			AP_CL_HoverColor: "#242424",
			...
			AP_CL_BGColor_Input: "#121212",
			AP_CL_BGColor_Divider: "#282830",
			rawWallpaperFileName: "default_darkmode",
			rawWallpaperFileType: ".jpg",
			AP_TX_Font_Primary: "Raleway",
			AP_TX_Font_Secondary: "Roboto",
			AP_TX_Font_Numbers: "Roboto",
...
// Sound Board Plus //
function SBP_PlayAudio(ItemID){
	var audioFile = document.getElementById(ItemID + "_Audio");
	audioFile.play();
	document.getElementById(ItemID).style.borderColor = "var(--Accent-Color)";
	audioFile.addEventListener("ended", function (){
		audioFile.currentTime = 0;
		document.getElementById(ItemID).style.borderColor = "var(--BGColor-Menus)";
	});
}
var SBP_PauseUnpause_State = 0;
function SBP_PauseUnpause(){
	if (SBP_PauseUnpause_State == 0){
		SBP_PauseAllSounds();
		document.getElementById("pageElement_SBP_PauseUnpause_Header").innerHTML = "Play all paused sounds";
		SBP_PauseUnpause_State = 1;
	} else {
		SBP_PlayAllSounds();
		document.getElementById("pageElement_SBP_PauseUnpause_Header").innerHTML = "Pause all playing sounds";
		SBP_PauseUnpause_State = 0;
	}
}

var SBP_Array_List_ItemCount_Titles = Object.keys(JSON.parse(localStorage.getItem("SBP_List_Titles_List 1"))).length

function SBP_PlayAllSounds(){
	var arrayLength = SBP_Array_List_ItemCount_Titles;
	for (a = 1; a != arrayLength; a++){
		var audioFile = document.getElementById("SBP_Item_" + a + "_Audio");
		if(audioFile.currentTime > 0){
			audioFile.play();
			document.getElementById("SBP_Item_" + a).style.borderColor = "var(--Accent-Color)";
			audioFile.addEventListener("ended", function (){
				audioFile.currentTime = 0;
				document.getElementById(ItemID).style.borderColor = "var(--BGColor-Menus)";
			});
	...
	//trigger_createToast("SBP_Play");
}

function SBP_PauseAllSounds(){
	var arrayLength = SBP_Array_List_ItemCount_Titles;
	for (a = 1; a != arrayLength; a++){
		var audioFile = document.getElementById("SBP_Item_" + a + "_Audio");
		audioFile.pause();
		document.getElementById("SBP_Item_" + a).style.borderColor = "var(--BGColor-Menus)";
	}
	//trigger_createToast("SBP_Pause");
}

function SBP_StopAllSounds(){
	var arrayLength = SBP_Array_List_ItemCount_Titles;
	for (a = 1; a != arrayLength; a++){
		var audioFile = document.getElementById("SBP_Item_" + a + "_Audio");
		audioFile.pause();
	...
	//trigger_createToast("SBP_Stop");
}

/*const systemTest_Lists= ["Category", "Category 1", "Category 2", "Category 3", "Category 4"]; //For testing only; Creates the category array
localStorage.setItem("SBP_Lists", JSON.stringify(systemTest_Lists)); //For testing only; Saves the category array*/

const systemTest_ListItem1_Titles = ["fillerData", "Lady Gaga - John Wayne", "The Weeknd - Blinding Lights", "Bruno Mars - Finesse"]; //For testing only; Creates the category array
//localStorage.setItem("SBP_List_Titles_List 1", JSON.stringify(systemTest_ListItem1_Titles)); //For testing only; Saves the category array
const systemTest_ListItem1_SoundURLs = ["fillerData", "Assets/Audio/Lady Gaga - John Wayne.mp3", "Assets/Audio/The Weeknd - Blinding Lights.mp3", "Assets/Audio/Bruno Mars - Finesse.mp3"]; //For testing only; Creates the category array
//localStorage.setItem("SBP_List_SoundURLs_List 1", JSON.stringify(systemTest_ListItem1_SoundURLs)); //For testing only; Saves the category array

let SBP_Array_List_Titles = [];
let SBP_Array_List_SoundURLs = [];
var SBP_Array_List_ItemCount;

function SBP_CheckFileExistence(){
	var ListItem_Titles = localStorage.getItem("SBP_List_Titles_List 1");
	if (ListItem_Titles){
		var SBP_Array_List_ItemCount = Object.keys(JSON.parse(localStorage.getItem("SBP_List_Titles_List 1"))).length; //Gets the length of the selected key
		var SBP_Array_List_Data = Object.values(JSON.parse(localStorage.getItem("SBP_List_Titles_List 1")));//Gets the data from selected key and temporarily stores it into variable
		for (a = 0; a != SBP_Array_List_ItemCount; a++){
			SBP_Array_List_Titles.push(SBP_Array_List_Data[a]); //Transfers all data from variable into category array
		} 
		console.log("Titles exists");
		
	} else {
		/*SBP_Array_List_Titles.push("fillerData"); //Adds the item "link" to the category array 
		localStorage.setItem("SBP_List_Titles_List 1", JSON.stringify(SBP_Array_List_Titles)); //Saves the category array into the selected key */
		console.log("Titles doesn't exist");
		let temp_SBP_Array_FillerData = ["fillerData"];
		localStorage.setItem("SBP_List_Titles_List 1", JSON.stringify(temp_SBP_Array_FillerData)); 
		window.location.href = "SBP_BoardEditor.html";
		location.reload();
		console.log("Titles doesn't exist");
	}
	
	var ListItem_SoundURLs = localStorage.getItem("SBP_List_SoundURLs_List 1");
	if (ListItem_SoundURLs){
		var SBP_Array_List_ItemCount = Object.keys(JSON.parse(localStorage.getItem("SBP_List_SoundURLs_List 1"))).length; //Gets the length of the selected key
		var SBP_Array_List_Data = Object.values(JSON.parse(localStorage.getItem("SBP_List_SoundURLs_List 1")));//Gets the data from selected key and temporarily stores it into variable
		for (a = 0; a != SBP_Array_List_ItemCount; a++){
			SBP_Array_List_SoundURLs.push(SBP_Array_List_Data[a]); //Transfers all data from variable into category array
			console.log(SBP_Array_List_SoundURLs);
		} 
		SBP_Generate_SoundButtons();
		if (PageName == "SBP_BoardEditor.html"){
			SBP_Generate_SoundButtons_TableView();
		}
	} else {
			let temp_SBP_Array_FillerData = ["fillerData"];
			localStorage.setItem("SBP_List_SoundURLs_List 1", JSON.stringify(temp_SBP_Array_FillerData)); 
			window.location.href = "SBP_BoardEditor.html";
			location.reload();
	}
...
function SBP_Generate_SoundButtons(){
	var SBP_Array_List_ItemCount_Titles = Object.keys(JSON.parse(localStorage.getItem("SBP_List_Titles_List 1"))).length;
	var SBP_Array_List_ItemCount_SoundURLs = Object.keys(JSON.parse(localStorage.getItem("SBP_List_SoundURLs_List 1"))).length;
	for (a = 1; a != SBP_Array_List_ItemCount_Titles; a++){
		var SoundButton_Div = document.createElement('div');
		SoundButton_Div.setAttribute("id", "SBP_Item_"+a);
		SoundButton_Div.setAttribute("onclick", "SBP_PlayAudio(this.id)");
		SoundButton_Div.classList.add("SBP_Item");
		SoundButton_Div.innerHTML = SBP_Array_List_Titles[a];
		document.getElementById("pageElement_ContentContainer").appendChild(SoundButton_Div);
		
		var SoundButton_Audio = document.createElement('audio');
		SoundButton_Audio.setAttribute("id", "SBP_Item_"+a+"_Audio");
		console.log(a);
		console.log(SBP_Array_List_SoundURLs);
		console.log(SBP_Array_List_SoundURLs[a]);
		SoundButton_Audio.setAttribute("src", SBP_Array_List_SoundURLs[a]);
		document.getElementById("pageElement_ContentContainer").appendChild(SoundButton_Audio);
	}
	/*for (b = 1; b != SBP_Array_List_ItemCount_Titles; b++){
		var SoundButton_Audio = document.createElement('audio');
		SoundButton_Audio.setAttribute("id", "SBP_Item_"+b+"_Audio");
		SoundButton_Audio.src = SBP_Array_List_SoundURLs[b];
		SoundButton_Audio.innerHTML = SBP_Array_List_Titles[b];
		document.getElementById("pageElement_ContentContainer").appendChild(SoundButton_Audio);
	}*/
}

function SBP_Generate_SoundButtons_TableView(){
	var SBP_Array_List_ItemCount_Titles = Object.keys(JSON.parse(localStorage.getItem("SBP_List_Titles_List 1"))).length;
	
	var table = document.createElement('table');
	table.classList.add("ShortcutEditor_Table");
	table.setAttribute("id", "content_table_"+"1");
	document.getElementById("pageElement_ContentContainer_TableView").appendChild(table);
	for (SBP_SelectedButton = 1; SBP_SelectedButton != SBP_Array_List_ItemCount_Titles; SBP_SelectedButton++){ //Loops the code for each category item stored in key_Index_Category

			
			var tableRow = table.insertRow(0);
			tableRow.classList.add("ShortcutEditor_Table_Row");
			tableRow.setAttribute("id", SBP_Array_List_Titles[SBP_SelectedButton]);
			
			var tableCell_Name = tableRow.insertCell(0);
			tableCell_Name.innerHTML = SBP_Array_List_Titles[SBP_SelectedButton];
			tableCell_Name.classList.add("ShortcutEditor_Table_Cell_Name");
			
			var tableCell_URL = tableRow.insertCell(1);
			tableCell_URL.innerHTML = SBP_Array_List_SoundURLs[SBP_SelectedButton];
			tableCell_Name.classList.add("ShortcutEditor_Table_Cell_URL");
			
			var tableCell_Ctrls = tableRow.insertCell(2);
			tableCell_Ctrls.classList.add("ShortcutEditor_Table_Ctrl");
			tableCell_Ctrls.setAttribute("id", "content_tableRows_Ctrls_"+SBP_SelectedButton);
			...
			tableCell_Ctrls_Peek.setAttribute("id", Renderer_Shortcut_URLArray[Renderer_Selected_Shortcut]);
			tableCell_Ctrls_Peek.setAttribute("onclick", "SE_TV_PreviewElement(this.id)");
			document.getElementById("content_tableRows_Ctrls_"+SBP_SelectedButton).appendChild(tableCell_Ctrls_Peek);
			*/
			var tableCell_Ctrls_Edit = document.createElement('img');
			tableCell_Ctrls_Edit.classList.add("ShortcutEditor_Table_Ctrl_Edit");
			tableCell_Ctrls_Edit.setAttribute("src", "Assets/Icons/iconNew_edit.png");
			tableCell_Ctrls_Edit.setAttribute("id", SBP_SelectedButton);
			tableCell_Ctrls_Edit.setAttribute("onclick", "SBP_EditItem(this.id)");
			document.getElementById("content_tableRows_Ctrls_"+SBP_SelectedButton).appendChild(tableCell_Ctrls_Edit);
			
			var tableCell_Ctrls_Delete = document.createElement('img');
			tableCell_Ctrls_Delete.classList.add("ShortcutEditor_Table_Ctrl_Edit");
			tableCell_Ctrls_Delete.setAttribute("src", "Assets/Icons/iconNew_delete.png");
			tableCell_Ctrls_Delete.setAttribute("id", SBP_SelectedButton);
			tableCell_Ctrls_Delete.setAttribute("onclick", "SBP_Confirm_DeleteItem(this.id)");
			/*tableCell_Ctrls_Delete.setAttribute("onclick", "SE_TV_DeleteElement_GetCategoryNumber(this.id), SE_TV_DeleteElement(parentNode.parentNode.id)");*/
			document.getElementById("content_tableRows_Ctrls_"+SBP_SelectedButton).appendChild(tableCell_Ctrls_Delete);
		}
	}

function SBP_BP_CreateItem(){
	var SBP_BP_ButtonTitle = document.getElementById("form_SBP_BE_ButtonTitle").value;
	var SBP_BP_ButtonURL = document.getElementById("form_SBP_BE_ButtonURL").value;
	if (SBP_BP_ButtonTitle != "" && SBP_BP_ButtonURL != ""){
		SBP_Array_List_Titles.push(SBP_BP_ButtonTitle);
		localStorage.setItem("SBP_List_Titles_List 1", JSON.stringify(SBP_Array_List_Titles));
		
		SBP_Array_List_SoundURLs.push("Assets/Audio/"+SBP_BP_ButtonURL);
		localStorage.setItem("SBP_List_SoundURLs_List 1", JSON.stringify(SBP_Array_List_SoundURLs));
...
var SBP_EditItem_SelectedItem;

function SBP_EditItem(arrayItem){
	SBP_EditItem_SelectedItem = arrayItem;
	trigger_Open_SubWindow("EditItem");
	document.getElementById("EditItem_SubwindowTitle").innerHTML ="Edit Sound Button '"+SBP_Array_List_Titles[arrayItem]+"'";
	console.log(arrayItem);
	console.log(SBP_Array_List_Titles[arrayItem]);
	document.getElementById("form_SBP_BP_EditItem_SoundButton_Title").value = SBP_Array_List_Titles[arrayItem];
	document.getElementById("form_SBP_BP_EditItem_SoundButton_URL").value = SBP_Array_List_SoundURLs[arrayItem];
}

function SBP_EditItem_SaveChanges(){
	if (document.getElementById("form_SBP_BP_EditItem_SoundButton_Title").value != "" && document.getElementById("form_SBP_BP_EditItem_SoundButton_URL").value != ""){
		SBP_Array_List_Titles[SBP_EditItem_SelectedItem] = document.getElementById("form_SBP_BP_EditItem_SoundButton_Title").value;
		SBP_Array_List_SoundURLs[SBP_EditItem_SelectedItem] = document.getElementById("form_SBP_BP_EditItem_SoundButton_URL").value;
		localStorage.setItem("SBP_List_Titles_List 1", JSON.stringify(SBP_Array_List_Titles));
		localStorage.setItem("SBP_List_SoundURLs_List 1", JSON.stringify(SBP_Array_List_SoundURLs));
		SBP_RefreshList();
		trigger_Close_SubWindow("EditItem");
	} else {
		trigger_createToast("Settings_FormEmpty");
	}
}

var SBP_DeleteItem_SelectedItem;

function SBP_Confirm_DeleteItem(arrayItem){
	open_Subwindow("Confirmation_DeleteItem");
	ItemName = SBP_Array_List_Titles[arrayItem];
	document.getElementById("pageElement_Confirmation_DeleteItem").innerHTML = "Are you sure you want to delete "+ItemName+"?";
	
	SBP_DeleteItem_SelectedItem = arrayItem;
}

function SBP_DeleteItem(){
	console.log(SBP_Array_List_Titles[SBP_DeleteItem_SelectedItem]);
	console.log(SBP_Array_List_SoundURLs[SBP_DeleteItem_SelectedItem]);
	console.log(SBP_Array_List_Titles.splice(SBP_DeleteItem_SelectedItem, 1));
	console.log(SBP_Array_List_SoundURLs.splice(SBP_DeleteItem_SelectedItem, 1));
	
	SBP_Array_List_Titles.splice(SBP_DeleteItem_SelectedItem, 1);
	SBP_Array_List_SoundURLs.splice(SBP_DeleteItem_SelectedItem, 1);
	console.log("DELETE");
	console.log(SBP_Array_List_Titles);
	console.log(SBP_Array_List_SoundURLs);
